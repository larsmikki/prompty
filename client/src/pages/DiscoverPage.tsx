...
export default function DiscoverPage() {
  ...
  const handleAdd = async (preset: typeof PRESET_PROMPTS[0]) => {
    const hasCategory = userCategories.some(
      c => c.name.toLowerCase() === preset.category.toLowerCase()
    )
    try {
      if (!hasCategory) await addCategory(preset.category)
      await addPrompt(preset.title, preset.text, preset.category)
      setAddedIds(prev => new Set(prev).add(preset.id))
      addToast(`Added "${preset.title}"`, 'success')
    } catch (err) {
      addToast(err instanceof Error ? err.message : 'Failed to add preset to library', 'error')
    }
  }

  const libraryKeys = useMemo(
    () => new Set(prompts.map(p => `${p.title} ${p.text}`)),
    [prompts]
  )
  const isAlreadyInLibrary = (preset: typeof PRESET_PROMPTS[0]) =>
    libraryKeys.has(`${preset.title} ${preset.text}`)
    ...
    <div>
      <div className="mb-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight mb-1 text-text">Discover prompts</h1>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="text-base font-bold mb-1 text-text">No results</h3>
          <p className="text-sm text-text2">Try a different search term or category.</p>
        </div>
      ) : grouped ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {presets.map(preset => (
                  <DiscoverCard
                    key={preset.id}
                    preset={preset}
                    added={addedIds.has(preset.id) || isAlreadyInLibrary(preset)}
                    onAdd={handleAdd}
                  />
                ))}
              </div>
...
function DiscoverCard({
  ...
  return (
    ...
    >
      <h3 className="text-base font-bold mb-2 leading-snug text-text">{preset.title}</h3>
      <p className="text-sm leading-relaxed mb-4 line-clamp-3 flex-1 text-text2">{preset.text}</p>
